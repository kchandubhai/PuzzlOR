...
def update_battleship(alist,blist):
    if len(blist) < 1:
        ...
        templist = [value for value in alist
        if value not in blist]
        return templist

def assign_sub(value,alist,blist):
    calcList = [[euclidean_distance(
    generate_coordinate(val,blist),
    generate_coordinate(value,blist)),val]
    for val in alist]
    sorted_calcList = sorted(calcList,
