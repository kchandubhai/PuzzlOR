...
def distanceCalc(factories,towns,xVal):
    assigned = []
    townAssigned = []
    scores = []
    for factory in factories:
        ...
        while not checkVal:
            if len(townAssigned) == len(factories):
                  distance = sum([euclidean_distance
                  (generate_coordinate(assigned[i][0],xVal),generate_coordinate(assigned[i][1],xVal)) * scores[i] *10 
...
def main():
    ...
    for area in possibleArea:
        ...
        outputList.append(distanceCalc(tempFactories,towns,xVal))
    outputList.sort
    print(outputList)
